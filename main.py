            ...
            if file_type == 'video':
                if ext == 'm3u8':
                    ...
                    try:
                        ...
                        os.remove(original_path)  # Delete the original M3U8 file
                        new_filename = output_filename
                    except subprocess.CalledProcessError as e:
                        os.remove(original_path)
                        return {'error': f'M3U8 to MP4 conversion failed: {str(e)}'}
                    except Exception as e:
                        os.remove(original_path)
                        return {'error': f'M3U8 processing error: {str(e)}'}
                else:
                    # Convert non-M3U8 videos to HLS
